...
export function AdminWorkflowTab({ project, collectionData }) {
  ...
  const isStepCompleted = (stepNumber) => {
    ...
    switch (stepNumber) {
      case 1:
        return !!(collectionData.description || collectionData.target_audience || collectionData.campaign_season);
      case 2:
        return !!(
    ...
    <div className="space-y-6">
                ...
                <div className="flex flex-col items-center flex-1">
                  <div
                    ...
                    className={`flex items-center justify-center w-10 h-10 rounded-full transition-all ${
                      isCompleted
                        ...
                        : isActive
                          ? 'bg-blue-600 dark:bg-blue-500 border-2 border-blue-300 dark:border-blue-400'
                  ...
                  >
                    {isCompleted ? (
                      ...
                      <span
                        className={`font-semibold ${isActive ? 'text-white text-base' : 'text-gray-500 dark:text-gray-400 text-sm'}`}
                      >
                        {step.number}
                      </span>
                    )}
                  </div>
                  <p className="mt-2 text-sm text-center text-gray-700 dark:text-gray-300">{step.title}</p>
                </div>
                {index !== steps.length - 1 && (
                  <div
                    className={`h-0.5 flex-1 mx-2 ${step.number < activeStep ? 'bg-blue-600 dark:bg-blue-500' : 'bg-gray-300 dark:bg-gray-700'}`}
                    style={{ minWidth: '20px' }}
                  />
                )}
              </div>
      ...
      <div className="flex items-center justify-between">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          <ChevronLeft size={18} />
          Previous
        </button>
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 dark:bg-blue-500 text-white hover:bg-blue-700 dark:hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          Next
