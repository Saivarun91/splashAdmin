...
import { promptMasterAPI } from '@/lib/api';
...
export default function PromptMasterPage() {
  ...
  const initializePromptsIfNeeded = async () => {
    try {
      ...
      const response = await promptMasterAPI.getPrompts(token);
      if (response.success && (!response.prompts || response.prompts.length === 0)) {
        // Database is empty, initialize default prompts
        const initResponse = await promptMasterAPI.initializePrompts(token);
  ...
  const handleCreate = async () => {
    try {
      ...
      const token = getToken();
      const response = await promptMasterAPI.createPrompt(formData, token);
      if (response.success) {
  ...
  const handleUpdate = async (promptId) => {
    try {
      ...
      const token = getToken();
      const response = await promptMasterAPI.updatePrompt(promptId, formData, token);
      if (response.success) {
  ...
  const handleDelete = async (promptId) => {
    ...
    try {
      const token = getToken();
      const response = await promptMasterAPI.deletePrompt(promptId, token);
      if (response.success) {
