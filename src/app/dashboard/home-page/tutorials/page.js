...
export default function TutorialsPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await homepageAPI.getPageContentAdmin('tutorials');
  ...
  const save = async () => {
    try {
      ...
      await homepageAPI.updatePageContent('tutorials', parsed);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Tutorials Page</h1>
        <p className="text-gray-600 dark:text-gray-400 mt-2">Edit tutorials header and videos (JSON).</p>
      </div>
      {message.text && (
